...
BACKUP_DIR="/home/xiaoliu/backups/dsms-backend-r6-${STAMP}"
SWITCHED=0
HAD_OLD_JAR=0
...
rollback_on_error() {
  ...
  echo "DSMS 后端 R6 安装失败，正在恢复安装前 JAR……" >&2
  ...
  echo "DSMS_R6_BACKEND_INSTALL_FAILED；退出码：${exit_code}" >&2
  exit "${exit_code}"
}
...
# 只检查变量是否存在，不打印 AK、SK、数据库密码和邮件应用专用密码。
...
if ! sudo grep -Eq '^PORTAL_REGISTRATION_ENABLED=true[[:space:]]*$' "${ENV_FILE}" || \
   ! sudo grep -Eq '^PORTAL_REGISTRATION_INVITE_CODE=[A-Za-z0-9_-]{8,64}[[:space:]]*$' "${ENV_FILE}"; then
  echo "朋友邀请码注册尚未配置，请先运行 deploy/configure-portal-registration-r5.sh。" >&2
  exit 1
fi

for mail_setting in \
  '^MAIL_ENABLED=true[[:space:]]*$' \
  '^MAIL_USERNAME=.+' \
  '^MAIL_APP_PASSWORD=.+' \
  '^MAIL_RECIPIENT=.+'; do
  if ! sudo grep -Eq "${mail_setting}" "${ENV_FILE}"; then
    echo "DSMS 邮件提醒尚未完整配置，请先运行 deploy/configure-mail-r6.sh。" >&2
    exit 1
  fi
done
...
echo "DSMS_R6_BACKEND_INSTALL_OK"
