  ...
  <div class="request-page">
    ...
    <template v-if="step === 0">
      ...
      <div class="medicine-grid">
        <article v-for="p in prescriptions" :key="p.prescriptionId" class="medicine-card" :class="{ selected: selected[p.prescriptionId] }">
          <div class="medicine-top"><el-checkbox v-model="selected[p.prescriptionId]" size="large"><strong>{{ p.medicineName }}</strong></el-checkbox><el-tag :type="(p.remainingDays || 0) <= 7 ? 'danger' : 'success'">剩{{ p.remainingDays || 0 }}天</el-tag></div>
          <p>{{ p.brandName || '通用药' }} · {{ p.specification }}</p>
          <div class="price-row"><span>参考价 <b>¥{{ money(p.referencePrice) }}/盒</b></span><span>建议补足约30天</span></div>
          <div v-if="selected[p.prescriptionId]" class="quantity"><span>需要盒数</span><el-input-number v-model="quantities[p.prescriptionId]" :min="1" :max="99" size="large" /><strong>小计 ¥{{ money((p.referencePrice || 0) * quantities[p.prescriptionId]) }}</strong></div>
        </article>
      </div>
    ...
    <template v-else>
      ...
      <el-card shadow="never" class="confirm-card">
        ...
        <el-checkbox v-model="confirmed" size="large" class="confirm-check">我已经核对药品、规格和盒数，确认提交给家人</el-checkbox>
      </el-card>
    </template>

    <div class="actions"><el-button v-if="step === 1" size="large" @click="step = 0">返回修改</el-button><el-button v-if="step === 0" type="primary" size="large" :disabled="!selectedItems.length" @click="goConfirm">下一步：核对信息</el-button><el-button v-else type="primary" size="large" :loading="submitting" :disabled="!confirmed" @click="submit">确认无误，提交申请</el-button></div>
  </div>
</template>
...
const step = ref(0); const reason = ref('LOW_STOCK'); const note = ref(''); const confirmed = ref(false); const submitting = ref(false)
...
const goConfirm = () => { if (!selectedItems.value.length) return ElMessage.warning('请至少选择一种药品'); confirmed.value = false; step.value = 1; window.scrollTo({ top: 0, behavior: 'smooth' }) }
const submit = async () => {
