  ...
  <div class="page">
    ...
    <el-dialog v-model="visible" title="登记代购订单" width="760px" destroy-on-close>
      <el-form label-position="top" size="large">
        <div class="form-grid"><el-form-item label="购药平台"><el-select v-model="order.purchasePlatform" filterable allow-create><el-option v-for="p in platforms" :key="p" :label="p" :value="p"/></el-select></el-form-item><el-form-item label="线上/线下"><el-radio-group v-model="order.purchaseChannel"><el-radio-button value="ONLINE">线上</el-radio-button><el-radio-button value="OFFLINE">线下</el-radio-button></el-radio-group></el-form-item><el-form-item label="下单时间"><el-date-picker v-model="order.orderTime" type="datetime" value-format="YYYY-MM-DDTHH:mm:ss"/></el-form-item><el-form-item label="预计到货时间"><el-date-picker v-model="order.expectedArrivalTime" type="datetime" value-format="YYYY-MM-DDTHH:mm:ss" placeholder="告诉家人预计何时送到"/></el-form-item></div>
...
const order=reactive({taskId:null,purchasePlatform:'京东健康',purchaseChannel:'ONLINE',orderTime:'',expectedArrivalTime:'',screenshotUrl:'',carrierCode:'jd',carrierName:'京东物流',trackingNo:'',note:'',items:[]})
...
const expectedArrivalText=()=>{const d=new Date();d.setDate(d.getDate()+3);const pad=n=>String(n).padStart(2,'0');return `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}T18:00:00`}
const openOrder=t=>{current.value=t;const c=content(t);Object.assign(order,{taskId:t.taskId,purchasePlatform:'京东健康',purchaseChannel:'ONLINE',orderTime:nowText(),expectedArrivalTime:expectedArrivalText(),screenshotUrl:'',carrierCode:'jd',carrierName:'京东物流',trackingNo:'',note:'',items:(c.items||[]).map(i=>({...i,unitPrice:Number(i.estimatedUnitPrice||0),expiryDate:tomorrowYear()}))});visible.value=true}
const carrierChanged=code=>{order.carrierName=carriers.find(c=>c.code===code)?.name||'其他'}
const uploadProof=async opt=>{uploading.value=true;try{const fd=new FormData();fd.append('file',opt.file);fd.append('category','ORDER_SCREENSHOT');const r=await request.post('/files/images',fd);order.screenshotUrl=r.data.url;ElMessage.success('截图已安全上传并等待归档')}finally{uploading.value=false}}
const saveOrder=async()=>{if(!order.purchasePlatform||!order.orderTime||!order.expectedArrivalTime)return ElMessage.warning('请填写平台、下单时间和预计到货时间');if(!order.screenshotUrl)return ElMessage.warning('请上传下单截图，让家人看到购买凭证');if(order.items.some(i=>!i.expiryDate))return ElMessage.warning('请填写每种药品的有效期');saving.value=true;try{await createFamilyOrder({...order,items:order.items.map(i=>({prescriptionId:i.prescriptionId,quantityBoxes:i.quantityBoxes,unitPrice:i.unitPrice,expiryDate:i.expiryDate}))});ElMessage.success('订单已登记，安心用药端已收到提醒');visible.value=false;load()}finally{saving.value=false}}
