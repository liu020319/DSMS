...
const capabilitiesRef=ref(),contactRef=ref(),verified=ref(false),verifying=ref(false),submitting=ref(false),challengeId=ref(''),challengeReadyAt=ref(0),success=reactive({inquiryNo:'',accessCode:''}),scrollPercent=ref(0),viewportWidth=ref(window.innerWidth)
const progressDrawer=ref(false),progress=ref({}),replyVerified=ref(false),replyVerifying=ref(false),replyChallengeId=ref(''),replyChallengeReadyAt=ref(0)
...
const createChallenge=async(target,readyAt)=>{const r=await authApi.challenge();target.value=r.data.challengeId;readyAt.value=humanChallengeReadyAt()}
const verify=async()=>{verifying.value=true;try{if(!challengeId.value)await createChallenge(challengeId,challengeReadyAt);await waitUntilHumanChallengeReady(challengeReadyAt.value);const r=await authApi.verify(challengeId.value);form.humanToken=r.data.humanToken;verified.value=true}catch(e){reportError(e,'验证失败');if(e.code!==429){challengeId.value='';challengeReadyAt.value=0;createChallenge(challengeId,challengeReadyAt).catch(()=>{})}}finally{verifying.value=false}}
const submit=async()=>{if(!form.contactName.trim()||!form.contactValue.trim())return ElMessage.warning('请填写称呼和联系方式');if(form.inquiryText.trim().length<10)return ElMessage.warning('请至少用10个字描述需求');if(!verified.value)return ElMessage.warning('请先完成人机验证');submitting.value=true;try{const r=await publicServiceApi.submitInquiry({...form});Object.assign(success,r.data)}catch(e){reportError(e);verified.value=false;form.humanToken='';challengeId.value='';challengeReadyAt.value=0;createChallenge(challengeId,challengeReadyAt).catch(()=>{})}finally{submitting.value=false}}
...
const resetForm=()=>{Object.assign(form,{contactName:'',contactValue:'',serviceType:'GUIDANCE',projectType:'',inquiryText:'',sourcePath:'/cloud-hub/#/services',humanToken:''});Object.assign(success,{inquiryNo:'',accessCode:''});verified.value=false;challengeId.value='';challengeReadyAt.value=0;createChallenge(challengeId,challengeReadyAt).catch(()=>{})}
...
const verifyReply=async()=>{replyVerifying.value=true;try{if(!replyChallengeId.value)await createChallenge(replyChallengeId,replyChallengeReadyAt);await waitUntilHumanChallengeReady(replyChallengeReadyAt.value);const r=await authApi.verify(replyChallengeId.value);reply.humanToken=r.data.humanToken;replyVerified.value=true}catch(e){reportError(e);if(e.code!==429){replyChallengeId.value='';replyChallengeReadyAt.value=0;createChallenge(replyChallengeId,replyChallengeReadyAt).catch(()=>{})}}finally{replyVerifying.value=false}}
const addMessage=async()=>{if(reply.messageText.trim().length<2)return ElMessage.warning('请填写留言内容');if(!replyVerified.value)return ElMessage.warning('请先完成人机验证');try{await publicServiceApi.addMessage({inquiryNo:query.inquiryNo,accessCode:query.accessCode,messageText:reply.messageText,humanToken:reply.humanToken});reply.messageText='';reply.humanToken='';replyVerified.value=false;replyChallengeId.value='';replyChallengeReadyAt.value=0;await queryProgress();ElMessage.success('留言已发送，邮件提醒将由服务器继续处理')}catch(e){reportError(e)}}
...
onMounted(()=>{createChallenge(challengeId,challengeReadyAt).catch(()=>{});addEventListener('scroll',onScroll,{passive:true});addEventListener('resize',onResize,{passive:true})})
