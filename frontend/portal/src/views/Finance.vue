  ...
  <div class="feature-page finance-studio">
    ...
    <template v-else-if="ledgerId">
      ...
      <section class="two-column finance-lower">
        <article class="panel"><div class="panel-head"><div><small>ACCOUNTS</small><h3>资金账户</h3></div><el-button text @click="accountDialog=true">新增账户</el-button></div><div class="account-list"><div v-for="a in accounts" :key="a.accountId"><span>{{ accountIcon[a.accountType] || '□' }}</span><div><b>{{a.accountName}}</b><small>{{ accountName[a.accountType] || a.accountType }}</small></div><strong>初始 ¥{{money(a.initialBalance)}}</strong></div></div></article>
...
const ledgerForm=reactive({ledgerName:''}),accountForm=reactive({accountName:'',accountType:'WECHAT',initialBalance:0})
...
const createLedger=async()=>{if(!ledgerForm.ledgerName.trim())return ElMessage.warning('请输入账本名称');try{const r=await financeApi.createLedger(ledgerForm);ledgerDialog.value=false;ledgerForm.ledgerName='';ledgerId.value=r.data.ledgerId;await loadLedgers();ElMessage.success('账本已创建')}catch(e){reportError(e)}}
